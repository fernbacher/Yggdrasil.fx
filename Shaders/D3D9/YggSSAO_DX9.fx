...
//
//  Depth convention (verified from debug screenshot):
//    Near geometry = LOW linearized depth value (dark)
//    Far geometry  = HIGH linearized depth value (light)
//    Sky           = ~1.0
//
//  Occlusion logic:
//    For a pixel at depth centerD, an occluder at sampleD occludes it
//    if sampleD < centerD (sample is CLOSER = in front = blocks ambient light)
//    diff = sampleD - centerD → negative for occluders
//    Thickness gate: reject if diff < -SSAOThickness (too far in front, wrong surface)
//    Bias gate:      reject if diff > -SSAOBias (too similar, self-occlusion)
//
//  All loops with tex2D manually unrolled (HLSL DX9 compiler requirement).
// =============================================================================
...
float PS_SSAO_Compute(float4 pos : SV_Position, float2 uv : TEXCOORD) : SV_Target
{
    ...
    float frameJ   = frac(float(SSAOFrame) / 64.0) * 64.0 * 0.61803398875;
